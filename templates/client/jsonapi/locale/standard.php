...
$entry_fcn = function (\Aimeos\M_Shop\Locale\Item\Iface $item) use ($fields, $target, $cntl, $action, $config) {
    $id = $item->get_id();
    $attributes = $item->to_array();
    $type = $item->get_resource_type();
    $opt_target = $this->config('client/jsonapi/url/target');
    $params = ['resource' => $type, 'id' => $id];
    $resource_params = ['locale' => $item->get_language_id(), 'currency' => $item->get_currency_id()];
    if (isset($fields[$type])) {
        $attributes = array_intersect_key($attributes, $fields[$type]);
    }
    $entry = ['id' => $id, 'type' => $type, 'links' => ['self' => ['href' => $this->url($target, $cntl, $action, $params, [], $config), 'allow' => ['GET']], 'resources' => ['href' => $this->url($opt_target, $cntl, $action, $resource_params, [], $config), 'allow' => ['OPTIONS']]], 'attributes' => $attributes];
    return $entry;
};
?>
{
...
if (isset($this->errors)) {
    ...
    if (is_map($items)) {
        foreach ($items as $locale_item) {
            $data[] = $entry_fcn($locale_item);
        }
    } else {
