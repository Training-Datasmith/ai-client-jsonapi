...
$entry_fcn = function (\Aimeos\M_Shop\Attribute\Item\Iface $item) use ($fields, $target, $cntl, $action, $config) {
    $id = $item->get_id();
    ...
    foreach ($item->get_property_items() as $prop_item) {
        $entry['relationships'][$type . '.property']['data'][] = ['id' => $prop_item->get_id(), 'type' => $type . '.property'];
    }
