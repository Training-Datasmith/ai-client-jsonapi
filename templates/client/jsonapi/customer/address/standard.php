...
$entry_fcn = function (\Aimeos\M_Shop\Customer\Item\Address\Iface $item) use ($fields, $target, $cntl, $action, $config) {
    $id = $item->get_id();
    $attributes = $item->to_array();
    $rtype = str_replace('/', '.', $item->get_resource_type());
    $params = ['resource' => 'customer', 'id' => $item->get_parent_id(), 'related' => 'address', 'relatedid' => $id];
    $basket_params = ['resource' => 'basket', 'id' => 'default', 'related' => 'address', 'relatedid' => 'delivery'];
    if (isset($fields[$rtype])) {
        $attributes = array_intersect_key($attributes, $fields[$rtype]);
    }
    $entry = ['id' => $id, 'type' => $rtype, 'links' => ['self' => ['href' => $this->url($target, $cntl, $action, $params, [], $config), 'allow' => ['DELETE', 'GET', 'PATCH']], 'basket.address' => ['href' => $this->url($target, $cntl, $action, $basket_params, [], $config), 'allow' => ['POST']]], 'attributes' => $attributes];
    return $entry;
};
?>
{
...
if (isset($this->errors)) {
    ...
    if (is_map($items)) {
        foreach ($items as $addr_item) {
            $data[] = $entry_fcn($addr_item);
        }
    } else {
