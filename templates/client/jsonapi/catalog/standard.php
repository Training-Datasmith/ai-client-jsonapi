...
$entry_fcn = function (\Aimeos\M_Shop\Catalog\Item\Iface $item) use ($fields, $target, $cntl, $action, $config) {
    if ($item->is_available() === false) {
        return [];
    }
    $id = $item->get_id();
    $type = $item->get_resource_type();
    $params = ['resource' => $type, 'id' => $item->get_id()];
    ...
    foreach ($item->get_children() as $cat_item) {
        if ($cat_item->is_available()) {
            $entry['relationships']['catalog']['data'][] = ['id' => $cat_item->get_id(), 'type' => 'catalog'];
        }
    }
...
$cat_fcn = function (\Aimeos\M_Shop\Catalog\Item\Iface $item, array $entry) use ($target, $cntl, $action, $config) {
    $params = ['resource' => 'catalog', 'id' => $item->get_id()];
...
if (isset($this->errors)) {
    ...
    $included = $this->jincluded($items, $fields, ['catalog' => $cat_fcn]);
    if (is_map($items)) {
