 ...
 * @subpackage JsonApi
 */
namespace Aimeos\Client\Json_Api\Basket\Coupon;
...
class Standard extends \Aimeos\Client\Json_Api\Basket\Base implements \Aimeos\Client\Json_Api\Iface
{
        ...
        try {
            ...
            if ($rel_id === '' || $rel_id === null) {
                ...
                foreach ($payload->data as $entry) {
                    ...
                    $this->controller->delete_coupon($entry->id);
                }
            } else {
                $this->controller->delete_coupon($rel_id);
            }
    ...
    public function post(Server_Request_Interface $request, Response_Interface $response): \Psr\Http\Message\Response_Interface
    {
        ...
        try {
            ...
            foreach ($payload->data as $entry) {
                if (!isset($entry->id)) {
                    throw new \Aimeos\Client\Json_Api\Exception('Coupon code is missing', 400);
                }
                $this->controller->add_coupon($entry->id);
            }
            $view->item = $this->controller->get();
            $status = 201;
        } catch (\Aimeos\M_Shop\Plugin\Provider\Exception $e) {
    ...
    {
        return $this->get_options_response($request, $response, 'DELETE,GET,OPTIONS,PATCH,POST');
    }
    /**
